/*
 * Modified for HW 6: see *** comments below
 */
...
public class MaxPQ<Key> implements Iterable<Key>, PQ<Key> // *** PQ is new
// interface
{
	...
	{
		// *** Note the following Comparator *inverts* the
		// String comparison.
		
		// Creating a MaxPQ with this Comparator results in
		// an implementation of a min-oriented heap!
		
		// The following is an example of a Java anonymous class:
		// we build an instance of a class with no name that
		// implements the Comparator<String> interface.
		//
		// This language feature was added in early Java (1.2)
		// to provide an easy way of implementing Swing
		// event handlers - objects that respond to mouse clicks,
		// etc. - without needing to define a separate class
		
		Comparator<String> minComp = new Comparator<String>() {
			public int compare (String s1, String s2)
			{
				return -s1.compareTo(s2); // s1 < s2 <=> s1 > s2
			}
		...
		// *** 
		//	Create a new MaxPQ with above Comparator => 
		//	a min-oriented heap!
		
		MaxPQ<String> pq = new MaxPQ<String>(minComp);
		
		// ***
		// Read Strings (words) from standard input (console)
		//	and enter each into pq.
		//	When - entered, remove max (here min due to minComp)
		//	 and echo to output.	
		// Keep going until no more input data, then announce
		//	how many items left in pq.
